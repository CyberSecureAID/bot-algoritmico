...
contract Tarifas is Initializable, UUPSUpgradeable {
    ...
    // ── Staking: parte de TODA comisión que va al bote común de los stakers ──
    uint16  public stakingBps;          // 2000 = 20 % general (editable por admin)
    uint16  public stakingFuturosBps;   // 5000 = 50 % de lo que genera Futuros
    address public tesoreriaStaking;    // contrato que recibe y reparte a los stakers
    uint16  public unstakeBps;          // comisión al retirar del staking (100 = 1 %)
    ...
    function initialize(address _principal) external initializer {
        ...
        bps[FUTUROS] = 4;   // 0,04 %
        bps[BOTS] = 0;
        stakingBps = 2000;         // 20 % general
        stakingFuturosBps = 5000;  // 50 % de Futuros
        unstakeBps = 100;   // 1 % al retirar del staking
    }
    ...
    function setStakingBps(uint16 b) external soloAdmin { if (b > 10000) revert Limite(); stakingBps = b; emit Config("stakingBps", 0, b, address(0)); }
    function setStakingFuturosBps(uint16 b) external soloAdmin { if (b > 10000) revert Limite(); stakingFuturosBps = b; emit Config("stakingFuturosBps", 0, b, address(0)); }
    function setTesoreriaStaking(address t) external soloAdmin { tesoreriaStaking = t; emit Config("tesoreriaStaking", 0, 0, t); }
    function setUnstakeBps(uint16 b) external soloAdmin { if (b > 1000) revert Limite(); unstakeBps = b; emit Config("unstakeBps", 0, b, address(0)); }
    /// Cuánto de un monto de comisión corresponde al bote de staking.
    function parteStaking(uint256 monto) external view returns (uint256) { return monto * stakingBps / 10000; }
