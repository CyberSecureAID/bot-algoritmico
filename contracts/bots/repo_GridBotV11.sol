...
contract GridBotV11 is Initializable, UUPSUpgradeable {
    ...
    address public owner2;          // segundo owner (recibe su parte del cobro de bots)
    uint256 public feeBps;          // comisión de swap (0.10%)
    uint256 public stakingBps;      // 2000 = 20% al staking
    uint256 public slippageMax;
    bool    public pausado;

    /* ---- Cobro de bots ---- */
    uint256 public costoBotUSD;     // 250 = $2.50 (2 decimales)
    uint256 public maxBots;         // 8
    mapping(address => uint32) public botsAbiertos;   // nº de bots activos por wallet
    mapping(address => uint40) public pagadoHasta;    // timestamp hasta el que la wallet está al día
    mapping(address => bool)   public cobroAuto;      // el usuario permite el cobro automático (perfil)
    ...
    struct Rejilla {
        address base; address quote; uint256 ordenQuote; uint256 ordenBase;
        uint256 posicionBase; uint256 costeQuote; uint256 volumenQuote; int256 gananciaQuote;
        uint128 tpUnitOut; uint128 slUnitOut; uint64 creadaEn; uint64 ultimaOpEn;
        uint32 comprasHechas; uint32 ventasHechas; uint32 ciclos; uint32 cooldownSeg;
        uint16 slippageBps; bool activa; bool pausadaPago; Nivel[] niveles;
        uint24 feeTier; uint8 modo; uint16 objetivoBps; uint16 factorBps; uint16 margenBps;
    ...
    function numDexes() external view returns (uint256) { return dexes.length; }
    function saludDex(uint256 i) external view returns (string memory nombre, bool activo, uint32 fallos, uint40 ultimoUso) {
        SwapLib.Dex storage d=dexes[i]; return (d.nombre,d.activo,d.fallos,d.ultimoUso);
    }
    ...
    function pagarMes() external payable noReentrada {
        _cobrarMes(msg.sender, msg.value);
    }
    ...
    function depositarGasPara(address u) external payable { _acreditarGas(u, msg.value); }
    ...
    function swap(address tokenIn, address tokenOut, uint24 feeTier, uint256 amountIn, uint256 minOut) external payable noReentrada returns (uint256 salida) {
        ...
        // Swap normal por DEX (con comisión)
        ...
        (salida,) = dexes.swapConFallback(dexPref, tin, tout, feeTier, tokenOut==address(0)?address(this):msg.sender, net, minOut);
    ...
    function cancelarRejilla(bytes32 k) public {
        require(duenoDe[k]==msg.sender);
        delete rejillas[k].niveles; rejillas[k].activa=false;
        if (botsAbiertos[msg.sender]>0) botsAbiertos[msg.sender]-=1;
        emit RejillaActiva(k,false);
    }
    function _cerrar(address u, bytes32 k) internal {
        ...
        if (vender>0) { recibido=_pull(r.base,u,vender); uint256 net=_comision(r.base,recibido); (qOut,)=dexes.swapConFallback(dexPref,r.base,r.quote,r.feeTier,u,net,0); _venta(r,recibido,qOut); r.ventasHechas+=1; }
        r.activa=false;
    ...
    function _paso(address u, address tin, address tout, uint24 fee, uint256 amt, uint256 minNivel, uint16 slip) internal returns (uint256 salida, uint256 entradaReal) {
        entradaReal=_pull(tin,u,amt);
        (uint256 qG,)=dexes.mejorCotizacion(dexPref,tin,tout,fee,entradaReal);
        require(qG>=minNivel);
        uint256 net=_comision(tin,entradaReal);
        uint256 minReal=(qG*net/entradaReal)*(BPS-slip)/BPS;
        (salida,)=dexes.swapConFallback(dexPref,tin,tout,fee,u,net,minReal);
    }
    ...
    function rejilla(bytes32 k) external view returns (Rejilla memory) { return rejillas[k]; }
    function estadoPago(address u) external view returns (uint32 bots, uint40 hasta, bool aldia, uint256 costoBNB) {
        return (botsAbiertos[u], pagadoHasta[u], alDia(u), costoBotBNB());
    }
    ...
    function setModulo(address m) external soloOwner { modulo=m; }
    ...
    function setMaxBots(uint256 n) external soloOwner { maxBots=n; }
    ...
    function setGasMinOp(uint256 w) external soloOwner { gasMinOp=w; }
    function transferirOwner(address n) external soloOwner { require(n!=address(0)); ownerPendiente=n; }
    function aceptarOwner() external { require(msg.sender==ownerPendiente); owner=ownerPendiente; ownerPendiente=address(0); }
    function agregarToken(address t, string calldata s) external soloOwner { require(t!=address(0)); if(!_vistoToken[t]){_vistoToken[t]=true; tokens.push(t);} tokenInfo[t]=TokenInfo(true,s); }
    function tokensListados() external view returns (address[] memory) { return tokens; }
    function rescatarResiduo(address t, address para) external soloOwner noReentrada { require(para!=address(0)); uint256 b=IERC20(t).balanceOf(address(this)); require(b>0); t.safeTransfer(para,b); }
}
