...
library SwapLib {
    ...
    function cotizar(Dex[] storage dexes, uint256 i, address tin, address tout, uint24 fee, uint256 amt) internal returns (uint256 out) {
    ...
    function mejorCotizacion(Dex[] storage dexes, uint256 pref, address tin, address tout, uint24 fee, uint256 amt)
        internal returns (uint256 out, uint256 idx)
    {
    ...
    function _intentar(address router, address tin, address tout, uint24 ft, address recipient, uint256 amountIn, uint256 minOut) private returns (bool) {
        IERC20L(tin).approve(router, 0);
        IERC20L(tin).approve(router, amountIn);
        try IV3Router(router).exactInputSingle(
            IV3Router.ExactInputSingleParams({
                tokenIn: tin, tokenOut: tout, fee: ft, recipient: recipient,
                deadline: block.timestamp, amountIn: amountIn, amountOutMinimum: minOut, sqrtPriceLimitX96: 0
            })
        ) returns (uint256) { return true; } catch { return false; }
    }
    function swapConFallback(
    ...
    ) internal returns (uint256 recibido, uint256 usado) {
        uint256 antes = IERC20L(tout).balanceOf(recipient);
        uint24[4] memory fees = [fee, uint24(500), uint24(2500), uint24(100)];
        for (uint256 pass = 0; pass < dexes.length; pass++) {
            ...
            for (uint256 f = 0; f < 4; f++) {
                if (f > 0 && fees[f] == fee) continue;
                if (_intentar(dexes[i].router, tin, tout, fees[f], recipient, amountIn, minOut)) {
