...
(async () => {
  let idi;
  try {
    idi = await import(J + 'idioma.js?v=142');
    idi.arrancarIdioma();
  } catch (e) {
...
/* ══════════════════════════════════════════════════════════════════════
   CINTA INFINITA CÓSMICA — llenado con las monedas y sus logos
   ══════════════════════════════════════════════════════════════════════
   Pide los logos a CoinGecko (misma API gratuita que ya usa el swap) y monta
   DOS sets idénticos en el track. El CSS anima translateX a -50% exacto, así
   que con los dos sets el bucle es perfecto e infinito. Sin precios: solo
   logo y símbolo. Si CoinGecko no responde, se usa una lista de reserva con
   los iconos que la web ya tiene, para que la cinta nunca quede vacía.
*/
(function () {
  var track = document.getElementById('pt-ticker-track');
  if (!track) return;

  // Monedas a mostrar (id de CoinGecko -> símbolo visible).
  var MONEDAS = [
    ['bitcoin','BTC'], ['ethereum','ETH'], ['binancecoin','BNB'], ['solana','SOL'],
    ['ripple','XRP'], ['cardano','ADA'], ['polkadot','DOT'], ['litecoin','LTC'],
    ['avalanche-2','AVAX'], ['chainlink','LINK'], ['cosmos','ATOM'], ['near','NEAR'],
    ['pancakeswap-token','CAKE'], ['uniswap','UNI'], ['aave','AAVE'], ['injective-protocol','INJ'],
    ['dogecoin','DOGE'], ['shiba-inu','SHIB'], ['bitcoin-cash','BCH'], ['filecoin','FIL'],
    ['ethereum-classic','ETC'], ['polygon','MATIC'], ['floki','FLOKI'], ['trust-wallet-token','TWT']
  ];

  function pinta(logos) {
    // Un set de monedas en HTML.
    function set() {
      return MONEDAS.map(function (m) {
        var img = logos[m[0]] || '';
        var ico = img
          ? '<img src="' + img + '" alt="" loading="lazy">'
          : '<span class="pt-coin-t-ph"></span>';
        return '<span class="pt-coin-t">' + ico + '<b>' + m[1] + '</b></span>';
      }).join('');
    }
    // DOS sets idénticos = bucle invisible a -50%.
    track.innerHTML = set() + set();
  }

  // Logos desde CoinGecko (la misma API gratuita del swap).
  var ids = MONEDAS.map(function (m) { return m[0]; }).join(',');
  fetch('https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=' + ids + '&per_page=250')
    .then(function (r) { return r.ok ? r.json() : null; })
    .then(function (arr) {
      var logos = {};
      if (arr) arr.forEach(function (c) { logos[c.id] = c.image; });
      pinta(logos);
    })
    .catch(function () { pinta({}); });   // sin conexión: monta igual, sin logos
})();
