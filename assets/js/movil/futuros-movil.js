...
export async function pintarFuturos(host, api) {
  ...
  host.innerHTML = `
    ...
    <div class="op-grid">
      ...
      <div class="op-form">
        ...
        <button class="op-sltoggle" id="op-tptoggle"><span>Take-profit <em>(opcional)</em></span><i>▾</i></button>
        <div class="op-field op-slbox" id="op-tpbox" style="display:none">
          <span>Take-profit</span><input id="op-in-tp" inputmode="decimal" placeholder="0.00"><b>${esc(_quote)}</b>
        </div>
        <div class="op-finfo">
        ...
        <button class="op-buy" id="op-buy">Long</button>
        <button class="op-sell" id="op-sell">Short</button>
      </div>
    </div>
  ...
  // ── FUTUROS: apalancamiento, modo margen, take-profit, liquidación ──
  var _lev = 10, _modo = 'aislado';
  const tpt = $('op-tptoggle'); if (tpt) tpt.onclick = () => { const box = $('op-tpbox'); const ab = box.style.display === 'none'; box.style.display = ab ? '' : 'none'; tpt.classList.toggle('on', ab); };
  const rng = $('fm-range'); if (rng) rng.oninput = () => { _lev = +rng.value; $('fm-lev').textContent = _lev + '×'; $('fm-levbtn-v').textContent = _lev + '×'; calcFut(); };
  const mIn = $('op-in-amt'); if (mIn) mIn.addEventListener('input', calcFut);
  function _liqDe(e, l, esL) { const f = 0.75; return esL ? e * (1 - f / l) : e * (1 + f / l); }
  function calcFut() {
    const margen = parseFloat(($('op-in-amt').value || '').replace(/,/g, '')) || 0;
    const px = _libro.precio || 0;
    if ($('fm-size')) $('fm-size').textContent = margen ? (margen * _lev).toLocaleString('en-US', { maximumFractionDigits: 1 }) + ' USDT' : '—';
    if ($('fm-liq')) $('fm-liq').textContent = (px && margen) ? '$' + _liqDe(px, _lev, _ladoFut === 'long').toLocaleString('en-US', { maximumFractionDigits: 1 }) : '—';
  }
  var _ladoFut = 'long';
  // Ventana explicativa aislado/cruzado.
  const fmMode = $('fm-mode');
  if (fmMode) fmMode.onclick = () => {
    const ov = document.createElement('div'); ov.className = 'fxm-modal';
    ov.innerHTML = '<div class="bg"></div><div class="card"><h4>Modo de margen</h4>' +
    ...
    ov.querySelectorAll('.opt').forEach((o) => o.onclick = () => { _modo = o.dataset.m; fmMode.textContent = _modo === 'aislado' ? 'Aislado' : 'Cruzado'; ov.remove(); });
  };
  const fmLevBtn = $('fm-levbtn'); if (fmLevBtn) fmLevBtn.onclick = () => rng && rng.focus();
  calcFut();
  ...
  $('op-buy').onclick = () => { _ladoFut = 'long'; ejecutarFut('op-buy'); };
  $('op-sell').onclick = () => { _ladoFut = 'short'; ejecutarFut('op-sell'); };
...
function ejecutarFut(btnId) {
  const margen = parseFloat(($('op-in-amt').value || '').replace(/,/g, '')) || 0;
  if (!margen) { const i = $('op-in-amt'); if (i) i.focus(); return; }
