...
async function abrir(clave, arg) {
  ...
  try {
    switch (clave) {
      ...
      case 'market':    { inyectarFixMarket(); const m = await import('../market.js?v=131'); m.abrirMarket && m.abrirMarket(); break; }
...
async function abrirMarketTab(tabId) {
  inyectarFixMarket();
  const m = await import('../market.js?v=131');
...
async function irA(tab) {
  ...
  if (tab === 'futuros') { const fm = await import('./futuros-movil.js?v=7'); fm.pintarFuturos(host, api()); return; }
}
...
export async function montarMovil(deps) {
  ...
  try { const tg = await import('../gridbot/panel-trigger.js?v=8'); tg.iniciarTriggerPanel && tg.iniciarTriggerPanel(); } catch (_) {}
