...
function abrirSwapCoinModal(lado) {
  ...
  if (!LOGO_ST.ok) _cargarLogosPrecios();
  // Reintentos de repintado: los logos llegan async y a veces el repintado
  // por _cmRepintar no se dispara en móvil. Se fuerza unas veces.
  [400, 1000, 2000, 3500].forEach((t) => setTimeout(() => { if (window._cmRepintar === pintar) pintar(); }, t));
  // Clic robusto por delegación (además del onclick de cada botón).
  const lst = $('scm-list');
  if (lst && !lst._deleg) {
    lst._deleg = true;
    lst.addEventListener('click', (e) => {
      const b = e.target.closest('.cm-coin');
      if (b && b.dataset.id) swElegir(lado, b.dataset.id);
    });
  }
